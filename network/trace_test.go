...
var (
	traceJobConfig            = common.RunnerConfig{}
	traceJobCredentials       = &common.JobCredentials{ID: -1}
	traceJobConfigOutputLimit = common.RunnerConfig{OutputLimit: 1}
)
...
func TestIgnoreStatusChange(t *testing.T) {
	jobInfoMatcher := generateJobInfoMatcher(traceJobCredentials.ID, common.Success, common.NoneFailure)

	mockNetwork := new(common.MockNetwork)
	defer mockNetwork.AssertExpectations(t)

	// expect to receive just one status
	mockNetwork.On("UpdateJob", traceJobConfig, traceJobCredentials, jobInfoMatcher).
		Return(common.UpdateSucceeded).Once()

	b, err := newJobTrace(mockNetwork, traceJobConfig, traceJobCredentials)
...
func TestJobAbort(t *testing.T) {
	...
	keepAliveUpdateMatcher := generateJobInfoMatcher(traceJobCredentials.ID, common.Running, common.NoneFailure)
	updateMatcher := generateJobInfoMatcher(traceJobCredentials.ID, common.Success, common.NoneFailure)

	mockNetwork := new(common.MockNetwork)
	defer mockNetwork.AssertExpectations(t)

	// abort while running
	mockNetwork.On("UpdateJob", traceJobConfig, traceJobCredentials, keepAliveUpdateMatcher).
		Return(common.UpdateAbort).Once()

	// try to send status at least once more
	mockNetwork.On("UpdateJob", traceJobConfig, traceJobCredentials, updateMatcher).
		Return(common.UpdateAbort).Once()
...
func TestJobOutputLimit(t *testing.T) {
	...
	b, err := newJobTrace(mockNetwork, traceJobConfigOutputLimit, traceJobCredentials)
	require.NoError(t, err)
	...
	mockNetwork.On("PatchTrace", traceJobConfigOutputLimit, traceJobCredentials, mock.Anything, mock.Anything).
	...
	mockNetwork.On("UpdateJob", traceJobConfigOutputLimit, traceJobCredentials, updateMatcher).
		Return(common.UpdateSucceeded).Once()
...
func TestJobMasking(t *testing.T) {
	...
	jobTrace, err := newJobTrace(mockNetwork, traceJobConfig, traceJobCredentials)
	require.NoError(t, err)
...
func TestJobFinishTraceUpdateRetry(t *testing.T) {
	...
	// accept just 3 bytes
	mockNetwork.On("PatchTrace", traceJobConfig, traceJobCredentials, []byte("My trace send"), 0).
		Return(3, common.UpdateSucceeded).Once()

	// retry when trying to send next bytes
	mockNetwork.On("PatchTrace", traceJobConfig, traceJobCredentials, []byte("trace send"), 3).
	...
	mockNetwork.On("PatchTrace", traceJobConfig, traceJobCredentials, []byte("send"), 9).
		Return(6, common.UpdateRangeMismatch).Once()

	// accept rest of trace
	mockNetwork.On("PatchTrace", traceJobConfig, traceJobCredentials, []byte("ce send"), 6).
...
func TestJobMaxTracePatchSize(t *testing.T) {
	...
	// expect just 5 bytes
	mockNetwork.On("PatchTrace", traceJobConfig, traceJobCredentials, []byte("My tr"), 0).
		Return(5, common.UpdateSucceeded).Once()

	// expect next 5 bytes
	mockNetwork.On("PatchTrace", traceJobConfig, traceJobCredentials, []byte("ace s"), 5).
		Return(10, common.UpdateSucceeded).Once()

	// expect last 3 bytes
	mockNetwork.On("PatchTrace", traceJobConfig, traceJobCredentials, []byte("end"), 10).
		Return(13, common.UpdateSucceeded).Once()
...
func TestJobIncrementalPatchSend(t *testing.T) {
	...
	finalUpdateMatcher := generateJobInfoMatcher(
		traceJobCredentials.ID, common.Success, common.NoneFailure)
	...
	wg.Add(1)
	mockNetwork.On("PatchTrace", traceJobConfig, traceJobCredentials, []byte("test trace"), 0).
		Return(10, common.UpdateSucceeded).Once().
		Run(func(args mock.Arguments) {
			wg.Done()
		})
	...
	mockNetwork.On("UpdateJob", traceJobConfig, traceJobCredentials, finalUpdateMatcher).
		Return(common.UpdateSucceeded).Once()
...
func TestJobIncrementalStatusRefresh(t *testing.T) {
	...
	incrementalUpdateMatcher := generateJobInfoMatcher(
		traceJobCredentials.ID, common.Running, common.NoneFailure)
	...
	wg.Add(1)
	mockNetwork.On("UpdateJob", traceJobConfig, traceJobCredentials, incrementalUpdateMatcher).
		Return(common.UpdateSucceeded).Once().
		Run(func(args mock.Arguments) {
			wg.Done()
		})
