...
func (c *CacheExtractorCommand) getClient() *network.CacheClient {
	if c.client == nil {
...
func (c *CacheExtractorCommand) download() (bool, error) {
	...
	logrus.Infoln("Downloading", filepath.Base(c.File), "from", formatter.CleanURL(c.URL))
...
func init() {
	RegisterCommand2("cache-extractor", "download and extract cache artifacts (internal)", &CacheExtractorCommand{
