...
// RunWithOutput provides a mock function with given fields: cmd, out
func (_m *MockExecutor) RunWithOutput(cmd ExecutorCommand, out io.Reader) error {
	ret := _m.Called(cmd, out)

	var r0 error
	if rf, ok := ret.Get(0).(func(ExecutorCommand, io.Reader) error); ok {
		r0 = rf(cmd, out)
	} else {
		r0 = ret.Error(0)
	}
