...
func (c DockerConfig) GetPullPolicy() ([]DockerPullPolicy, error) {
	// Default policy is always
	if c.PullPolicy == nil {
		return []DockerPullPolicy{PullPolicyAlways}, nil
	}

	validPullPolicy := map[DockerPullPolicy]bool{
	...
	for _, p := range c.PullPolicy {
		_, ok := validPullPolicy[p]
		if !ok {
			return []DockerPullPolicy{}, fmt.Errorf("unsupported docker-pull-policy: %v", p)
		}
	}
