...
func NewMethod(path string, data map[string]interface{}) (vault.AuthMethod, error) {
	newData, err := auth_methods.FilterAuthenticationData(requiredPayloadFields, allowedPayloadFields, data)
	if err != nil {
...
func (a *method) Authenticate(client vault.Client) error {
	authPath := fmt.Sprintf("auth/%s/login", a.path)
	authPayload := a.data
