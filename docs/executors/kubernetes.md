...
| `init_objects` | This setting is in Alpha. Create a list of objects before creating the build pod. Allows to patch the objects' spec. See [Creating Kubernetes objects using the spec patch functionality (Alpha)](#creating-kubernetes-objects-using-the-spec-patch-functionality-alpha). |
...
> As of [GitLab Runner 16.0](https://gitlab.com/gitlab-org/gitlab-runner/-/merge_requests/4099) the `patch` field supports variable expansion.
...
### Creating Kubernetes objects using the spec patch functionality (Alpha)

> [Introduced](https://gitlab.com/gitlab-org/gitlab-runner/-/merge_requests/4099) in GitLab Runner 16.3.
...
With this functionality a list of objects can be created prior to creating the build pod in a similar fashion to how the [Pod Spec](#overwrite-generated-pod-specifications-alpha) functionality works.

The created objects will be cleaned up alongside the other Runner resources.

| Setting | Description |
|---------|-------------|
| `name` | Name given to the created Kubernetes object. |
| `kind` | The type of the Kubernetes object to be created. Currently limited to `PersistentVolumeClaim` and `PersistentVolume`. Use [this issue](https://gitlab.com/gitlab-org/gitlab-runner/-/issues/29659) to let us know what other types you might need. |
| `persist` | This configuration is used to persist the Kubernetes object created. When set to `true`, the Kubernetes object won't be cleaned up at the end of the job. It is then up to the user to make sure that those resources are deleted when they are not needed anymore |
| `patch_path` | Path to the file that defines the changes to apply to the final `PodSpec` object before it is generated. The file must be a JSON or YAML file. |
| `patch` | A JSON or YAML format string that describes the changes which must be applied to the final `PodSpec` object before it is generated. |
| `patch_type` | The strategy the runner uses to apply the specified changes to the `PodSpec` object generated by GitLab Runner. The accepted values are `merge`, `json`, and `strategic`. |

#### Create a PVC for each build job

To create a [PersistentVolumeClaim](https://kubernetes.io/docs/concepts/storage/persistent-volumes/) for each build job we first need a `Volume`.
For testing purposes the simplest one is a volume that mounts a directory on its host:
...
We can then create a Claim for that volume for each build pod by configuring it in the `config.toml`:

```toml
[[runners.kubernetes.init_objects]]
name = "builds-pvc-$CI_COMMIT_SHA"
...
We also need to patch our `Pod` so that it uses this `PVC`:
    ...
    volumes:
        - name: builds
          persistentVolumeClaim:
            claimName: builds-pvc-$CI_COMMIT_SHA
'''
```

To confirm that the bindings worked, start a job and describe the pod. The volume should be listed as follows:
...
Volumes:
  builds:
    ...
    ClaimName:  builds-pvc-01db179860cd4c87f6ea3e675eb0498e1a1c2309
    ReadOnly:   false
...
```

Note that this example is rather limiting since the relationship between a `PersistentVolume` and a `PersistentVolumeClaim` is 1:1 but in some setups
creating a `PVC` will automatically provision a `PV` which makes this functionality very useful.
