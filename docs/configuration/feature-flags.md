...
| `FF_USE_LEGACY_PROCESS_TERMINATION` | `false` | ✗ | 12.9 | Switches back from `SIGTERM` to `SIGKILL` and to old termination strategy for process termination on Unix systems when Shell executor is used |
