...
func (e *executor) sendSIGTERMToContainerProcs(ctx context.Context, id string) error {
	e.Debugln("Emitting SIGTERM to processes in container", id)

	execConfig := types.ExecConfig{
		Tty:          true,
		AttachStderr: true,
		AttachStdout: true,
		Cmd:          []string{"sh", "-c", shells.ContainerSigTermScript},
	}

	exec, err := e.client.ContainerExecCreate(ctx, id, execConfig)
	if err != nil {
		e.Warningln("Failed to exec create to container:", err)
		return err
	}

	resp, err := e.client.ContainerExecAttach(ctx, exec.ID, types.ExecStartCheck{})
	if err != nil {
		e.Warningln("Failed to exec attach to container:", err)
		return err
	}
	defer resp.Close()
	...
	if err != nil {
		e.Warningln("Failed to read from attached container:", err)
		return err
	}
