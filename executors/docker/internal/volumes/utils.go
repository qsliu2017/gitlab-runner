...
var (
	directoryNotAbsoluteErr = errors.New("build directory needs to be an absolute path")
	directoryIsRootPathErr  = errors.New("build directory needs to be a non-root path")
)
...
func IsHostMountedVolume(volumeParser parser.Parser, dir string, volumes ...string) (bool, error) {
	if !volumeParser.Path().IsAbs(dir) {
		return false, directoryNotAbsoluteErr
	}

	if volumeParser.Path().IsRoot(dir) {
		return false, directoryIsRootPathErr
	}
