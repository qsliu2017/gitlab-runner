// Code generated by mockery v1.0.0. DO NOT EDIT.

// This comment works around https://github.com/vektra/mockery/issues/155

package parser

import mock "github.com/stretchr/testify/mock"
import path_helpers "gitlab.com/gitlab-org/gitlab-runner/helpers/path"

// MockParser is an autogenerated mock type for the Parser type
type MockParser struct {
	mock.Mock
}

// ParseVolume provides a mock function with given fields: spec
func (_m *MockParser) ParseVolume(spec string) (*Volume, error) {
	ret := _m.Called(spec)

	var r0 *Volume
	if rf, ok := ret.Get(0).(func(string) *Volume); ok {
		r0 = rf(spec)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*Volume)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(spec)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Path provides a mock function with given fields:
func (_m *MockParser) Path() path_helpers.Path {
	ret := _m.Called()

	var r0 path_helpers.Path
	if rf, ok := ret.Get(0).(func() path_helpers.Path); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(path_helpers.Path)
		}
	}

	return r0
}
